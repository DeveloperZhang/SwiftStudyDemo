...
class GCDViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    let dataArray = ["0串行同步", "1串行异步", "2并发同步", "3并发异步", "4主队列同步", "5主队列异步", "6GCD线程之间通信", "7GCD队列组", "8GCD重复执行", "9GCD栅栏", "10数据安全(数据同步)", "11资源竞争信号量"]
    ...
    func safeData() {
        ...
        for i in 1...10 {
            queue.async {
                ...
                number += 1
                print("并发异步\(i)    \(Thread.current)   label is    \(queue.label) number is \(number)")
                objc_sync_exit(self)
            }
        }
    }
